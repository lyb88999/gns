  ...
  <div class="h-full flex flex-col p-8 bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm dark:shadow-none overflow-hidden flex-1 flex flex-col transition-all duration-300">
       ...
       >
           ...
           <el-table-column prop="recipient" :label="t('logs.recipient')" />
           <el-table-column prop="status" :label="t('common.status')">
                <template #default="{ row }">
                    <el-tag 
                       :type="
                           row.status === 'SUCCESS' ? 'success' : 
                           (row.status === 'blocked' || row.status === 'BLOCKED' ? 'warning' : 
                           (row.status === 'skipped' || row.status === 'SKIPPED' ? 'info' : 'danger'))
                       " 
                       effect="dark" 
                       round 
                       size="small"
            ...
            <el-table-column prop="sentAt" :label="t('logs.time')">
                <template #default="{ row }">
    ...
    <el-dialog v-model="dialogVisible" :title="t('common.details')" width="600px" class="rounded-xl overflow-hidden !bg-white dark:!bg-gray-800 dark:!text-white">
        <el-descriptions :column="1" border class="mt-4">
            <el-descriptions-item :label="t('tasks.name')">{{ currentLog.taskName }}</el-descriptions-item>
            <el-descriptions-item :label="t('tasks.taskId')">{{ currentLog.taskId }}</el-descriptions-item>
            <el-descriptions-item :label="t('tasks.channels')">{{ currentLog.channel }}</el-descriptions-item>
            <el-descriptions-item :label="t('logs.recipient')">{{ currentLog.recipient }}</el-descriptions-item>
            <el-descriptions-item :label="t('common.status')">
                <el-tag 
                    :type="
                        currentLog.status === 'SUCCESS' ? 'success' : 
                        (currentLog.status === 'blocked' || currentLog.status === 'BLOCKED' ? 'warning' : 
                        (currentLog.status === 'skipped' || currentLog.status === 'SKIPPED' ? 'info' : 'danger'))
                    "
                >
                    {{ 
            ...
            <el-descriptions-item :label="t('logs.time')">{{ new Date(currentLog.sentAt).toLocaleString() }}</el-descriptions-item>
            <el-descriptions-item :label="t('logs.error')" v-if="currentLog.errorMessage">
